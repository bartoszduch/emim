...
def start_menu():
    input_box = pygame.Rect(maze_width * cell_size // 4, maze_height * cell_size // 2 - 20, 140, 32)
    ...
    while not start_game:
        ...
        screen.fill(BLACK)
        # Renderuj instrukcje
        start_text = font.render("Wpisz czas gry (w sekundach) i nacisnij Enter", True, WHITE)
        screen.blit(start_text, (maze_width * cell_size // 4 - 100, maze_height * cell_size // 2 - 60))
...
def end_menu(levels_completed):
    ...
    while running:
        ...
        screen.fill(BLACK)
        end_text = font.render(f"Koniec gry! Ukończono poziomów: {levels_completed}", True, WHITE)
        restart_text = font.render("Naciśnij Enter, aby zagrać ponownie, lub Esc, aby wyjść.", True, WHITE)
        screen.blit(end_text, (maze_width * cell_size // 4 - 100, maze_height * cell_size // 2 - 20))
        screen.blit(restart_text, (maze_width * cell_size // 4 - 100, maze_height * cell_size // 2 + 40))
